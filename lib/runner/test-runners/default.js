...
class DefaultRunner {
  ...
  async runTestSuite(modulePath, modules) {
    ...
    if (!this.argv['launch-url']) {
      ...
      console.log('before of addTestSuiteResults, will run exportResults', new Date());
      // eslint-disable-next-line no-console
      this.globalReporter.addTestSuiteResults(this.currentSuite.reporter.exportResults());
      // eslint-disable-next-line no-console
      console.log('after of addTestSuiteResults', new Date());
    }
  ...
  async reportResults() {
    // eslint-disable-next-line no-console
    console.log('Before reportResults', new Date());
    if (!this.isTestWorker()) {
      // eslint-disable-next-line no-console
      console.log('Running reportResults', new Date());
      this.printGlobalResults();
      await this.globalReporter.save();
      // eslint-disable-next-line no-console
      console.log('After reportResults', new Date());
    }
  }
