...
class TestSuite {
  ...
  sendReportToParentWorker() {
    ...
    console.log('[sendReportToParentWorker] starting', new Date(), this.context.getSuiteName());
    if (this.settings.use_child_process && typeof process.send === 'function') {
      // eslint-disable-next-line no-console
      console.log('[sendReportToParentWorker] sending message', new Date(), this.context.getSuiteName());
      ...
      // eslint-disable-next-line no-console
      console.log('[sendReportToParentWorker] message sent', new Date(), this.context.getSuiteName());
    ...
    // eslint-disable-next-line no-console
    console.log('[sendReportToParentWorker] end', new Date(), this.context.getSuiteName());
  }

  testSuiteFinished(failures) {
    // eslint-disable-next-line no-console
    console.log('[testSuiteFinished] starting', new Date(), this.context.getSuiteName());
    this.reporter.testSuiteFinished();
    this.currentRunnable = null;

    if (Concurrency.isWorker()) {
      // eslint-disable-next-line no-console
      console.log('[testSuiteFinished] before sendReportToParentWorker', new Date(), this.context.getSuiteName());
      this.sendReportToParentWorker();
      // eslint-disable-next-line no-console
      console.log('[testSuiteFinished] after sendReportToParentWorker', new Date(), this.context.getSuiteName());
    }

    NightwatchEventHub.emit(TestSuiteHook.finished, this.reporter.testResults.eventDataToEmit);
    // eslint-disable-next-line no-console
    console.log('[testSuiteFinished] end', new Date(), this.context.getSuiteName());
