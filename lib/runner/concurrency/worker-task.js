...
class WorkerTask extends EventEmitter {
  ...
  async runWorkerTask(colors, type) {
    ...
    port2.onmessage = ({data: result}) => {
      ...
      switch (result.type) {
        case 'testsuite_finished':
          // eslint-disable-next-line no-console
          console.log(`[testsuite_finished - TRIGGER - before] Task ${this.task_label}`,  new Date());

          result.itemKey = this.label,
          this.emit('message', result);
          // eslint-disable-next-line no-console
          console.log(`[testsuite_finished - TRIGGER - after] Task ${this.task_label}`,  new Date());
    ...
    return new Promise((resolve, reject) => {
      // eslint-disable-next-line no-console
      console.log(`[START] Task ${this.task_label} starting at index ${this.index}`,  new Date());

      setTimeout(() => {
        this.piscina.run({argv: this.argv, port1}, {transferList: [port1]})
          .catch(err => {
            // eslint-disable-next-line no-console
            console.log(`[ERR] Task ${this.task_label} ${err}`, new Date());
            err;
          })
          .then(failures => {
            // eslint-disable-next-line no-console
            console.log(`[THEN] Task ${this.task_label}`, new Date());
            // Log output
            if (this.settings.disable_output_boxes) {
              ...
              console.log(boxen(this.task_output.join('\n'), {
                title: `────────────────── ${failures ? symbols.fail : symbols.ok} ${this.task_label}`,
                padding: 1,
                borderColor: 'cyan'
              }));
            }
            // eslint-disable-next-line no-console
            console.log(`[END] Task ${this.task_label} finished with ${failures ? 'FAILURE' : 'SUCCESS'}`, new Date());
            if (failures) {
              return reject(new Error(`Task ${this.task_label} failed`));
            }
            resolve();
          });
